...
async function seed() {
  ...
  for (const card of createdCards) {
    ...
    for (let i = 0; i < attackCount; i++) {
      ...
      // Only add the attack if it hasn't been assigned yet
      if (!assignedAttacks.has(attack.id)) {
        assignedAttacks.add(attack.id); // Mark this attack as assigned
        cardAttackPromises.push(card.addAttack(attack).catch(err => {
          if (err.name !== 'SequelizeUniqueConstraintError') {
            console.error(`Error adding attack to card ${card.id}:`, err);
          }
        }));
      }
    }
